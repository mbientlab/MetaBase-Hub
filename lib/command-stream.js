...
async function start(options, config, cache, cacheFile) {
  ...
  for(let it of devices) {
    ...
    for(let s of Object.keys(config['sensors'])) {
      winston.info("Configuring the: ", s);
      if (!(s in SensorConfig)) {
        ...
        let modelNum = MetaWear.mbl_mw_metawearboard_get_model(d.board);
        let packed = true
        if (modelNum.key == 'METAMOTION_S') {
          d.board.modelNumber = 8
          packed = false
        }
        let state = await DataCapture.createState((handler) => MetaWear.mbl_mw_datasignal_subscribe(SensorConfig[s].signal(d.board, packed), ref.NULL, handler), s, options);
