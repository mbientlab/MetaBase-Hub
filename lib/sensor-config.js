...
module.exports = {
    ...
    "Accelerometer": new SettingHz("x-axis (g),y-axis (g),z-axis (g)", (board, config) => {
        let range = MetaWear.AccBoschRange.enums[closest([2.0, 4.0, 8.0, 16.0], config['range'])]
        console.log(range)
        MetaWear.mbl_mw_acc_set_range(board, 'range' in config ? range : 5);
        if (board.modelNumber == '8') {
            let odr = MetaWear.AccBmi270Odr.enums[closest([0.78, 1.56, 3.12, 6.25, 12.5, 25.0, 50.0, 100.0, 200.0], config['odr'])]
            console.log(odr)
            MetaWear.mbl_mw_acc_bmi270_set_odr(board, 'odr' in config ? odr : 1);
        } else {
            let odr = MetaWear.AccBmi160Odr.enums[closest([0.78, 1.56, 3.12, 6.25, 12.5, 25.0, 50.0, 100.0, 200.0], config['odr'])]
            console.log(odr)
            MetaWear.mbl_mw_acc_bmi160_set_odr(board, 'odr' in config ? odr : 1);
        }
    ...
    "Gyroscope" : new SettingHz("x-axis (deg/s),y-axis (deg/s),z-axis (deg/s)", (board, config) => {
        let odr = 'odr' in config ? MetaWear.GyroBoschOdr.enums[closest([25.0, 50.0, 100.0, 200.0], config['odr'])] : 0;
        console.log(odr)
        let range = 'range' in config ? MetaWear.GyroBoschRange.enums[closest([2000.0, 1000.0, 500.0, 250.0, 125.0], config['range'])] : 0;
        console.log(range)
        if (board.modelNumber == '8') {
            MetaWear.mbl_mw_gyro_bmi270_set_range(board, range);
            MetaWear.mbl_mw_gyro_bmi270_set_odr(board, odr);
            MetaWear.mbl_mw_gyro_bmi270_write_config(board);
        } else {
            MetaWear.mbl_mw_gyro_bmi160_set_range(board, range);
            MetaWear.mbl_mw_gyro_bmi160_set_odr(board, odr);
